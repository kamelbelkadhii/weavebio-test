...
def ingest_data():
    ...
    for reference in data['reference']:
        ...
        citation_node = Node('Citation', type=citation['@type'], name=validate_data(citation, '@name'), volume=citation.get('@volume'), date=citation.get('@date'), first=citation.get('@first'), last=citation.get('@last'), 
...
while True:
    # this one is for the scheduler to keep running
    pass
